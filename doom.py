...
score=0
level=1
levels=[[[200,350,50,50],[400,300,50,50],[600,350,50,50]],
...
while True:
    for event in pygame.event.get():
        ...
        if event.type==pygame.KEYDOWN and event.key==pygame.K_SPACE:
            if player.bottom>=h: player_vel=jump_strength
    player_vel+=gravity
    player.y+=player_vel
    if player.bottom>h: player.bottom=h; player_vel=0
    for ob in obstacles: ob.x-=speed
    if obstacles and obstacles[0].right<0: obstacles.pop(0)
    if len(obstacles)<len(levels[level-1]):
        ...
        for x,y,w_,h_ in lvl_data:
            if x>last_x: obstacles.append(pygame.Rect(x,y,w_,h_))
    win.fill((135,206,235))
    pygame.draw.rect(win,(255,0,0),player)
    for ob in obstacles: pygame.draw.rect(win,(0,0,0),ob)
